...
EntityBearwojacknest = ig.Entity.extend({
	size: {x: 120, y: 65},
	offset: {x: 0, y: 0},
	maxVel: {x: 0, y: 0},
	storeMaxVel: {x: 0, y: 0},
	...
	nameMe: function(){
		this.name = "bearwojackNest" + ig.game.bearwojackNestCount;
		ig.game.bearwojackNestCount++;
	},
	offScreen: function(){
		var xClear = false;
		var yClear = false;
	...
	spawnWojack: function(){
		...
		if (!this.pause && 
			...
			console.log('Bear wojack spawned' );	
			var whichFlip = this.pos.x < 0 ? true : false;
			ig.game.spawnEntity( EntityBearwojack, this.pos.x, this.pos.y, {myNest: this.name, flip: whichFlip});
			this.setRespawnTime();
			this.spawnCount++;
		}
	},
...
ig.EntityPool.enableFor( EntityBearwojacknest );
});
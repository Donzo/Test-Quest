...
EntityFountainblock = ig.Entity.extend({
	size: {x: 32, y: 36},
	maxVel: {x: 0, y: 0},
	offset: {x: 0, y: -4},
	...
	activating: false,
	deactivating: false,
	animSheet: new ig.AnimationSheet( 'media/blocks/fountainblock.png', 32, 32 ),
	rightFrame: null,
	zIndex: -1,
	setBricks: false,
	pauseTime: 10,
	...
	activated: function(){
		...
		//Rain Again?
		if (this.rainAgain){
			this.rainAgainTimer.set(this.pauseTime);
		}
		this.deactivated = false;
		this.active = true;
		ig.game.fountain = "off";
		ig.game.faucetOffNoise();
	},
	rainAgainNow: function(){
	...
	init: function( x, y, settings ) {
		...
		this.rainAgainTimer = new ig.Timer(0);
		this.proximityCheckTimer = new ig.Timer(0);
	},
	reset: function( x, y, settings ) {
		this.parent( x, y, settings );
		this.active = false;
		this.rainAgain = false;
		this.activating = false;
	...
	paused: function(){
		...
		this.rainAgainTimer.pause();
		//Get pause frame
		this.pauseFrame = this.currentAnim.frame;
		this.pause = true;
	},
	unpaused: function(){
		this.rainAgainTimer.unpause();
	...
	update: function() {		
		...
		//Rain Again
		if (this.rainAgain && this.rainAgainTimer.delta() > 0 && !this.deactivated){
			this.rainAgainNow();
		}
		//Feel the other faucets
		if (ig.game.fountain == "off" && !this.active){
			this.deactivated = false;
			this.active = true;
			//Rain Again?
			if (this.rainAgain){
		...
		else if (ig.game.fountain == "on" && this.active){
			this.deactivated = true;
			this.active = false;
			this.activating = false;
		}
...
ig.EntityPool.enableFor( EntityFountainblock );
});
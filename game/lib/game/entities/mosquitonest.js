...
EntityMosquitonest = ig.Entity.extend({
	size: {x: 64, y: 64},
	offset: {x: 0, y: 0},
	maxVel: {x: 0, y: 0},
	storeMaxVel: {x: 0, y: 0},
	...
	nameMe: function(){
		this.name = "mosquitoNest" + ig.game.mosquitoNestCount;
		ig.game.mosquitoNestCount++;
	},
	...
	spawnMosquito: function(){
		...
		if (!this.pause && this.respawnTimer.delta() >= 0){
			...
			ig.game.spawnEntity( EntityMosquito, randomSpawnX, randomSpawnY, {myNest: this.name, flip: whichFlip});
			this.setRespawnTime();
		}
	},
...
EntityMosquito = ig.Entity.extend({
	size: {x: 20, y: 20},
	offset: {x: 22, y: 20},
	...
	animSheets: {
		mosquito: new ig.AnimationSheet( 'media/enemies/mosquito.png', 64, 64 )
	},
	...
	init: function( x, y, settings ) {
		...
		this.anims.fly = new ig.Animation( this.animSheets.mosquito, .1, [0,1,2,3,4,5] );
		this.anims.flyFast = new ig.Animation( this.animSheets.mosquito, .05, [0,1,2,3,4,5] );
		this.anims.damaged = new ig.Animation( this.animSheets.mosquito, .1, [0,1,2,3,4,5] );
		this.anims.idle = new ig.Animation( this.animSheets.mosquito, 1, [0]);
	...
	nameMe: function(){
		this.name = "mosquito" + ig.game.mosquitoCount;
		ig.game.mosquitoCount++;
	},
	...
	offScreenCheck: function(){
		if (this.flip && this.pos.x < 0 - this.size.x && this.boundryTimer.delta() > 0 ||  !this.flip && this.pos.x > ig.game.collisionMap.width * ig.game.collisionMap.tilesize && this.boundryTimer.delta() > 0 ){
			console.log(this.name + " failed offscreen check and is dead now.");
			this.kill();
		}
	},
	boundaries: function(){
		if (this.pos.y > ig.system.height * 1.5 + ig.game.screen.y){
			console.log(this.name + " knocked off screen and dead.");
			this.kill();
		}
	},
	...
	ig.EntityPool.enableFor( EntityMosquitonest );
	ig.EntityPool.enableFor( EntityMosquito );
});